...
namespace CmkCable.API.Controllers
{
    ...
    public class EmailsController : ControllerBase
    {
        ...
        [HttpGet("test-sendgrid-config")]
        public IActionResult TestSendGridConfig()
        {
            try
            {
                Console.WriteLine("=== TestSendGridConfig called ===");
                ...
                var configStatus = new
                {
                    ...
                    {
                        SENDGRID_API_KEY = !string.IsNullOrEmpty(sendGridKey) ? $"{sendGridKey.Substring(0, Math.Min(10, sendGridKey.Length))}..." : "Not set",
                        SENDGRID_FROM_EMAIL = sendGridFromEmail ?? "Not set",
                        SENDGRID_FROM_NAME = sendGridFromName ?? "Not set"
                    },
                    configuration = new
                    {
                        SendGrid_ApiKey = !string.IsNullOrEmpty(configApiKey) ? $"{configApiKey.Substring(0, Math.Min(10, configApiKey.Length))}..." : "Not set",
                        SendGrid_FromEmail = configFromEmail ?? "Not set",
                        SendGrid_FromName = configFromName ?? "Not set"
                    },
                    finalValues = new
                    {
                        ApiKey = !string.IsNullOrEmpty(sendGridKey) ? "From Environment" : (!string.IsNullOrEmpty(configApiKey) ? "From Config" : "Not Available"),
                        FromEmail = !string.IsNullOrEmpty(sendGridFromEmail) ? sendGridFromEmail : configFromEmail ?? "webcmkkablo@gmail.com",
                        FromName = !string.IsNullOrEmpty(sendGridFromName) ? sendGridFromName : configFromName ?? "CMK KABLO"
                    },
                    status = (!string.IsNullOrEmpty(sendGridKey) || !string.IsNullOrEmpty(configApiKey)) ? "Configured" : "Not Configured",
                    timestamp = DateTime.UtcNow
                };
                
                Console.WriteLine($"SendGrid config test completed: {configStatus.status}");
                return Ok(configStatus);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"SendGrid config test failed: {ex.Message}");
                Console.WriteLine($"Stack trace: {ex.StackTrace}");
